...
class ClassificationAlgorithms:
    ...
    def forward_feature_selection(self, max_features, X_train, y_train):
    # Initialize lists to track the order of selected features and their corresponding accuracy scores.
        feature_selection_order = []
        feature_accuracy_history = []
        chosen_features = []
        
        # Instantiate the classification algorithm helper.
        classification_algo = ClassificationAlgorithms()
        
        # Initialize the previous best accuracy.
        previous_best_accuracy = 0

        # Iteratively select features up to the maximum specified.
        for feature_iteration in range(max_features):
            print(f"Iteration: {feature_iteration}")

            # Determine which features have not been selected yet.
            remaining_features = list(set(X_train.columns) - set(chosen_features))
            best_accuracy = 0
            best_feature_candidate = ""

            # Evaluate each remaining feature by temporarily adding it to the chosen features.
            for feature in remaining_features:
                candidate_feature_set = chosen_features.copy()  # Shallow copy is sufficient here.
                candidate_feature_set.append(feature)

                # Train a decision tree using the candidate feature set.
                (
                    predictions_train,
                    predictions_test,
                    probabilities_train,
                ...
                # If this feature improves performance, update the best candidate.
                if accuracy > best_accuracy:
                    best_accuracy = accuracy
                    best_feature_candidate = feature

            # Add the best performing feature to the chosen feature list.
            chosen_features.append(best_feature_candidate)
            previous_best_accuracy = best_accuracy
            feature_selection_order.append(best_feature_candidate)
            feature_accuracy_history.append(best_accuracy)

        return chosen_features, feature_selection_order, feature_accuracy_history
